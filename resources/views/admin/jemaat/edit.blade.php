...
<body class="bg-slate-50 font-sans text-slate-900">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1">
            <header class="bg-white border-b border-slate-200 p-4 flex justify-between items-center shadow-sm sticky top-0 z-10">
                <h2 class="text-xl font-semibold text-slate-800">Edit Data Jemaat</h2>
                ...
                <div class="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                        <div class="flex justify-between items-center">
                            <div>
                                <h3 class="text-lg font-bold text-slate-800">Ubah Identitas</h3>
                                <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold">Update data: {{ $jemaat->nama_lengkap }}</p>
                            </div>
                            <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold">ID: #{{ $jemaat->id }}</span>
                        </div>
                    </div>

                    <!-- Gunakan Method PUT untuk Update di Laravel -->
                    <form action="{{ route('admin.jemaat.update', $jemaat->id) }}" method="POST" class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $jemaat->nama_lengkap) }}" required 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $jemaat->tempat_lahir) }}" required 
                                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir', $jemaat->tanggal_lahir) }}" required 
                                        class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                                    <span id="umur_badge" class="absolute right-10 top-1/2 -translate-y-1/2 bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-1 rounded">
                                        {{ $jemaat->umur }} Tahun
                                    </span>
                                </div>
                        ...
                        <!-- INPUT HIDDEN: Umur tetap dikirim ke backend -->
                        <input type="hidden" id="real_umur" name="umur" value="{{ old('umur', $jemaat->umur) }}">
                            ...
                            <div>
                                ...
                                <div class="flex gap-6 mt-1">
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="radio" name="jenis_kelamin" value="L" {{ old('jenis_kelamin', $jemaat->jenis_kelamin) == 'L' ? 'checked' : '' }} required 
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="radio" name="jenis_kelamin" value="P" {{ old('jenis_kelamin', $jemaat->jenis_kelamin) == 'P' ? 'checked' : '' }} required 
                                            class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-300">
                            ...
                            <div>
                                ...
                                <select name="rayon" required 
                                    ...
                                    @for ($i = 1; $i <= 10; $i++)
                                        ...
                                        <option value="{{ $val }}" {{ old('rayon', $jemaat->rayon) == $val ? 'selected' : '' }}>
                                            {{ $val }}
                                        </option>
                                    @endfor
                                </select>
                        ...
                        <div>
                            ...
                            <select name="status" required 
                                class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition bg-white">
                                @foreach(['Aktif', 'Pindah', 'Meninggal', 'Pasif'] as $st)
                                    <option value="{{ $st }}" {{ old('status', $jemaat->status) == $st ? 'selected' : '' }}>
                                        {{ $st == 'Pasif' ? 'Pasif (Tidak Aktif)' : $st }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="pt-6 flex flex-col md:flex-row gap-3">
                            <button type="submit" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg shadow-lg shadow-emerald-200 transition active:scale-[0.98] flex justify-center items-center">
    ...
    <script>
        ...
        // Load awal tidak disembunyikan karena sudah ada data dari database
    </script>
